...
<div class="breadcrumb_cus">
    ...
    <ol class="breadcrumb">
			...
			<?php $brand_name= App\Models\Category::where('id',$id_)->get() ?>
			@foreach($brand_name as $item)
            <?php $brands_count = App\Models\Product::cateCount($item->id); ?>
                <li><a href="{{url('showAllCategory')}}" title="Category">Category</a></li>
				<li><a href="{{url('showCategory/'.$item->id)}}" title="{{$item->name}}">{{$item->name}} ({{$brands_count}})</a></li>
			@endforeach
        </ol>	
    </div><!-- end container -->
...
<div class="content_wrapper">
    <div class="container">
        <div class="row">
        @forelse($categoy_product as $row)
        ...
        <div style="clear:both;padding-top:20px;text-align:center;">
            <div class="pagination">{{$categoy_product->render()}}</div>
        </div>
