...
<div class="breadcrumb_cus">
    ...
    <ol class="breadcrumb">
            ...
            <li class="active">All Product Category</li>
        </ol>	
    </div><!-- end container -->
        ...
        <div class="content">
            ...
            @foreach($category_list as $row)
                <div class="col-md-4 col-sm-4">
                    <div class="event_box">
                        <a href="{{url('showCategory/'.$row->id)}}" title="{{$row->name}}">
                            <img src="{{asset('uploads/categories/'.$row->image)}}" alt="{{$row->name}}" class="img-responsive" />
                        </a>
                        <div class="event_title">
                            <?php $brands_count = App\Models\Product::cateCount($row->id); ?>
                            <h3><a href="{{url('showCategory/'.$row->id)}}" title="{{$row->name}}">{{$row->name}}
                            ( @if(isset($brands_count))
                                        {{$brands_count}}
                                    @else
                                        0
                                    @endif )<i class="fa fa-plus-circle"></i></a></h3>
                        </div><!-- end event_title -->
                    </div><!-- end event_box -->
                </div><!-- end item -->
            @endforeach
